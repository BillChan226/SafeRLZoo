...
class PPOBuffer:
    ...
    def finish_path(self, last_val=0, last_val_safe=0):
        ...
        self.adv_safe_buf[path_slice] = core.discount_cumsum(deltas_safe, 1)
        
        # the next line computes rewards-to-go, to be targets for the value function
        self.ret_buf[path_slice] = core.discount_cumsum(rews, self.gamma)[:-1]
        # print("rews_safe", rews_safe)
        self.ret_safe_buf[path_slice] = core.discount_cumsum(rews_safe, 1)[:-1]
...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def compute_loss_pi(data):
        ...
        adv_safe = data['adv_safe']
        adv_pre = ac.adv_safe(torch.cat((obs, act), dim=1))
        ...
        adv_safe = adv_safe.mean().item()
        adv_pre = adv_pre.mean().item()
        clipped = ratio.gt(1+clip_ratio) | ratio.lt(1-clip_ratio)
        clipfrac = torch.as_tensor(clipped, dtype=torch.float32).mean().item()
        pi_info = dict(kl=approx_kl, ent=ent, cf=clipfrac, adv_safe=adv_safe, adv_pre=adv_pre)
    ...
    def compute_loss_v(data):
        ...
        # print("ret_safe", ret_safe)
        return ((ac.v(obs) - ret)**2).mean(), ((ac.v_safe(obs_safe) - ret_safe)**2).mean()
    
    def compute_loss_adv(data):
        obs, act, adv_safe = data['obs'], data['act'], data['adv_safe']
        # print(np.shape(obs))
        # print(np.shape(act))

        return ((ac.adv_safe(torch.cat((obs, act), dim=1)) - adv_safe)**2).mean()
    ...
    adv_safe_optimizer = Adam(ac.adv_safe.parameters(), lr=vf_lr)
    ...
    def update():
        ...
        adv_l_old_safe = compute_loss_adv(data)
        adv_l_old_safe = adv_l_old_safe.item()
        ...
        for i in range(train_v_iters*2):
            adv_safe_optimizer.zero_grad()
            loss_adv_safe = compute_loss_adv(data)
            loss_adv_safe.backward()
            mpi_avg_grads(ac.adv_safe)
            adv_safe_optimizer.step()

        # Log changes from update
        kl, ent, cf, adv_safe, adv_pre = pi_info['kl'], pi_info_old['ent'], pi_info['cf'], pi_info['adv_safe'], pi_info['adv_pre']
        logger.store(LossPi=pi_l_old, LossV=v_l_old, LossAdv=adv_l_old_safe,
                     KL=kl, Entropy=ent, ClipFrac=cf, AdvSafe=adv_safe,AdvPre=adv_pre,
    ...
    def safety_constraint(obs, act):
        # Define your neural network model here and compute safety probability v.
        # print(obs)
        # print(act)
        # print("cat", torch.cat((obs, act), dim=0))
        safety_advantage = ac.adv_safe(torch.cat((obs, act), dim=0))
        # print(safety_advantage)
        return safety_advantage

    def safety_guard_newton(obs, act_initial, h=-1, max_iterations=40, tol=1e-4, beta = 0.01):
        act_initial = torch.Tensor(act_initial)
        obs = torch.Tensor(obs)
        act = act_initial.clone().requires_grad_(True)
    
        for it in range(max_iterations):
            
            v = safety_constraint(obs, act)
            grad_v = torch.autograd.grad(v, act)[0]
        
            if v >= h:
                return act, it, 0  # Safety constraint is satisfied
            
            delta_act = (v - h) / (grad_v + 0.0001)
            
            act = act - beta * delta_act
            
            if torch.norm(delta_act) < tol:      
                return act, it, v - h  # Convergence
        
        dist = v - h
        
        return act, it, dist  # Return the action found after max_iterations
    
    def safety_guard_bisection(obs, act_original, h=-1, tol=1e-4, max_iterations=100):
        act_original = torch.Tensor(act_original)
        obs = torch.Tensor(obs)
        
        lower_bound = torch.zeros_like(act_original)
        upper_bound = act_original.clone()
        feasible_action = upper_bound.clone()
        it = 0
        for it in range(max_iterations):
            candidate_action = (lower_bound + upper_bound) / 2.0
            if safety_constraint(obs, candidate_action) >= h:
                feasible_action = candidate_action
                lower_bound = candidate_action
            else:
                upper_bound = candidate_action

            if torch.norm(upper_bound - lower_bound) < tol:
                break

        return feasible_action, it
    
    def safety_guard_shrinking(obs, act_original, h=-1, tol=1e-4, max_iterations=100):
        act_original = torch.Tensor(act_original)
        obs = torch.Tensor(obs)
        ...
        upper_bound = act_original.clone()
        feasible_action = upper_bound.clone()
        if safety_constraint(obs, act_original) >= h:
            return act_original
        else:
            candidate_action = act_original
            for s in range(max_iterations):
                candidate_action = candidate_action - torch.Tensor([0.002]*act_dim[0])
                if safety_constraint(obs, candidate_action) >= h:
    ...
    def safety_guard_gradient(obs, act_original, h=-1, 
                         learning_rate=0.05, max_iterations=20, tol=1e-4):
        
        act_original = torch.Tensor(act_original)
        obs = torch.Tensor(obs)
        act = act_original.clone().requires_grad_(True)
        it = 0
        for it in range(max_iterations):
            v = safety_constraint(obs, act)
            
            if v >= h:
                # print("satisfyied at ", it)
                return act, it, 0  # Safety constraint is satisfied
            act.retain_grad()
            ac.adv_safe.zero_grad()
            # loss = torch.norm(v - h)  # Minimize the difference from the threshold h
            v = safety_constraint(obs, act)
            v.backward(retain_graph=True)
            grad = act.grad
            print("v: ", v)
            # Compute the gradient of the loss with respect to the action
            
            # grad = torch.autograd.grad(loss, act)[0]
            
            # Normalize the gradient with its norm (L2 norm)
            # grad_norm = torch.norm(grad)
            
            grad_norm = torch.abs(torch.max(grad.cpu()))
            
            
            if grad_norm > 0:
                grad = grad / (grad_norm + 1e-8)

            # Update the action using gradient descent
            act = act - learning_rate * grad
            # print(act)
            # Ensure that the new action remains within the feasible range
            act = torch.clamp(act, min=action_min, max=action_max)
    ...
    for epoch in range(epochs):
        ...
        safe_epsilon = max(safe_epsilon - 0.3, 0.3)
        ...
        for t in range(local_steps_per_epoch):
            ...
            # print("a", a)
            safety_lower_bound = -0.8 * (v_safe - (1 - safe_epsilon))
            a_guarded, it, dist = safety_guard_gradient(o, a, h=safety_lower_bound)
            # print("a_guarded", a_guarded)
            a = a_guarded.detach().numpy()
            ...
            # if d == True or (ep_len == max_ep_len):
            #     if ep_pen < 25:
            #         r_safe = 1
            # ep_sum += r_safe

            ep_len += 1
            # penalty = min(safety_constraint(o, a) - safety_lower_bound, 0)
            penalty = - cost #min(dist, 0)
            
            r_lag = r_goal + 0.1 * penalty#  + 1000* min(v_safe - (1 - safe_epsilon), 0)

            # save and log
            # store(self, obs, act, rew, val, logp, rew_safe, val_safe, logp_safe)
            buf.store(o, a, r_lag, v, logp, r_safe, v_safe)
            logger.store(Optim_dist=dist)
            logger.store(Proj_iter=it)
            logger.store(VVals=v)
            logger.store(VVals_safe=v_safe)
            logger.store(lower_bound=safety_lower_bound)
        ...
        #     for te in range(int(local_steps_per_epoch/4)): #*8):
        ...
        logger.log_tabular('LossAdv', average_only=True)
        ...
        logger.log_tabular('AdvSafe', average_only=True)
        logger.log_tabular('AdvPre', average_only=True)
        logger.log_tabular('lower_bound', with_min_and_max=True)
        logger.log_tabular('Proj_iter', average_only=True)
        logger.log_tabular('Optim_dist', with_min_and_max=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='JakaSafeGymEnv-v1')
