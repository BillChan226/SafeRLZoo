...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    initial = np.deg2rad(demo_epochs[0][0])
    ...
    demo_act = []
    demo_obs = []
    # print("initial postion", o)
    # print("here")
    # Main loop: collect experience in env and update/log each epoch
    for epoch, demo_epoch in enumerate(demo_epochs):
        epoch_ended = False
        o, ep_ret, ep_len = env.reset(), 0, 0 
        for st in range(len(demo_epoch)):
            # o = np.deg2rad(demo_epoch[st])
            a = np.deg2rad(demo_epoch[st])
            next_o, r, d, _ = env.step(a)
                        
            demo_act.append(a.tolist())
            demo_obs.append(o.tolist())
    ...
    demo = {"obs": demo_obs, "act": demo_act}
    with open("demo_live_jaka.pickle", "wb") as file:
        pickle.dump(demo, file)
    input("save live replay steps of traj to file")
    return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='gcl_jaka')
    ...
    for args.seed in range(0, 1):#0,10
        ...
        model, env = ppo(lambda : gym.make(args.env, debug_mode=False, action_repeat=10), actor_critic=core.MLPActorCritic,
